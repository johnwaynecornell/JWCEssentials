...
usage() {
    cat <<EOF
Usage:
  ./newage_build_managed.sh REPO_DIR [Debug|Release|Both|All] [--fresh] [--clean]

Examples:
  ./newage_build_managed.sh JWCEssentials Debug
  ./newage_build_managed.sh JWCCommandSpawn Release
  ./newage_build_managed.sh CrystalCatalystLibrary Both --fresh
  ./newage_build_managed.sh JWCCommandSpawn All --fresh --clean
...
Notes:
  managed bins from REPO_DIR will be created.
...
while [ "$#" -gt 0 ]; do
    case "$1" in
        ...
        --*)
            echo "[newage_build_managed] ERROR: Unknown option: $1" >&2
            usage >&2
            exit 1
            ;;
        ...
        *)
            if [ -z "$REPO_DIR" ]; then
                ...
                echo "[newage_build_managed] ERROR: Unexpected argument: $1" >&2
                usage >&2
                exit 1
            fi
            ;;
...
if [ -z "$BUILD_MODE" ]; then
    if [ -n "${NewAge_Lane:-}" ]; then
        ...
        echo "[newage_build_managed] Inferred BUILD_MODE from NewAge_Lane: $BUILD_MODE"
    else
        BUILD_MODE="Debug"
    fi
fi
